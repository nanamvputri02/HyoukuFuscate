...
try:
    ...
    if (
        ...
        print("\x1b[1m\x1b[92m" + chiyo("[+] dependencies installed"))
        sys.exit("\x1b[1m\x1b[92m" + chiyo("[+] run the program again"))
        ...
        print(
            "\x1b[1m\x1b[31m"
            + chiyo("[!] something error occured while installing dependencies")
        )
        sys.exit(
            "\x1b[1m\x1b[31m"
            + chiyo("maybe pip isn't installed or requirements.txt file not available?")
        )
...
def check_update():
    LATEST_VER = requests.get(
        "https://raw.githubusercontent.com/nanamvputri02/HyoukuFuscate/main/.version"
    ).text.strip()
...
def main():
    args = parse_args()
    if check_update():
        print(RED + chiyo("[!] update available"))
        print(LIGRE + chiyo("[+] updating..."))
        update()
        print(LIGRE + chiyo("[+] successfully updated..."))
        sys.exit(LIGRE + chiyo("run the program again"))
